...
3dDeconvolve -input pb03.$subj.r*.blur+tlrc.HEAD                           \
    ...
    -stim_file 13 $topdir/feature_analysis/boyManual_HRFconv.txt -stim_label 13 boyManual \
    -stim_file 14 $topdir/feature_analysis/dogManual_HRFconv.txt -stim_label 14 dogManual \
    -fout -tout -x1D XplusBoyDogManual.xmat.1D -xjpeg XplusBoyDogManual.jpg \
    -x1D_uncensored XplusBoyDogManual.nocensor.xmat.1D \
    -fitts fitts.BoyDogManual.${subj} \
    -errts errts.BoyDogManual.${subj} \
    -bucket stats.BoyDogManual.${subj} \
    -jobs 12 
done
