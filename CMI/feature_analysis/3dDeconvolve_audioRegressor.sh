...
3dDeconvolve -input pb03.$subj.r*.blur+tlrc.HEAD                           \
    ...
    -stim_file 13 $topdir/feature_analysis/mirrms_HRFconvolved.txt -stim_label 13 audioRMS \
    -fout -tout -x1D XplusAudioRMS.xmat.1D -xjpeg XplusAudioRMS.jpg                                \
    -x1D_uncensored XplusAudioRMS.nocensor.xmat.1D                                     \
    -fitts fitts.AudioRMS.$subj                                                     \
    -errts errts.AudioRMS.${subj}                                                   \
    -bucket stats.AudioRMS.$subj \
    -jobs 12 
done
