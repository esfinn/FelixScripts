...
3dDeconvolve -input pb03.$subj.r*.blur+tlrc.HEAD                           \
    ...
    -stim_file 14 $topdir/regressors/ambiguous_events_regressor.txt -stim_label 14 ambig \
    -fout -tout -x1D XAudioAmbig.xmat.1D -xjpeg XAudioAmbig.jpg                                \
    -x1D_uncensored XAudioAmbig.nocensor.xmat.1D                                     \
    -fitts fitts.AudioAmbig.$subj                                                     \
    -errts errts.AudioAmbig.${subj}                                                   \
    -bucket stats.AudioAmbig.$subj \
    -jobs 12 
done
