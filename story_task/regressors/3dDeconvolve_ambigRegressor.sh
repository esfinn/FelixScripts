...
3dDeconvolve -input pb03.$subj.r*.blur+tlrc.HEAD                           \
    ...
    -stim_file 13 $topdir/regressors/ambiguous_events_regressor.txt -stim_label 13 ambig \
    -fout -tout -x1D XplusAmbig.xmat.1D -xjpeg XplusAmbig.jpg                                \
    -x1D_uncensored XplusAmbig.nocensor.xmat.1D                                     \
    -fitts fitts.Ambig.$subj                                                     \
    -errts errts.Ambig.${subj}                                                   \
    -bucket stats.Ambig.$subj \
    -jobs 12 
done
